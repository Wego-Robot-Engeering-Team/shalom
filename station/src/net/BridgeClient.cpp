...
BridgeClient::BridgeClient(QString host, quint16 port, QObject *parent)
    : gcs::robot::RobotLink(parent), host_(std::move(host)), port_(port)
{
    clock_.start();
...
void BridgeClient::onConnected()
{
    ...
    for (const auto *ch : {gcs::ch::kPose, gcs::ch::kBattery, gcs::ch::kSystem,
                           gcs::ch::kSafety, gcs::ch::kNav, gcs::ch::kPlan,
                           gcs::ch::kTrail, gcs::ch::kArm, gcs::ch::kApriltag,
                           gcs::ch::kMission, gcs::ch::kWaypoints, gcs::ch::kMap,
                           gcs::ch::kPreview, gcs::ch::kCaptureSpool,
                           gcs::ch::kHealth})
...
void BridgeClient::handlePublish(const Envelope &env)
{
    ...
    if (ch == QLatin1String(gcs::ch::kPose)) {
    ...
    } else if (ch == QLatin1String(gcs::ch::kBattery)) {
    ...
    } else if (ch == QLatin1String(gcs::ch::kSafety)) {
    ...
    } else if (ch == QLatin1String(gcs::ch::kArm)) {
        QList<double> q;
    ...
    } else if (ch == QLatin1String(gcs::ch::kMission)) {
    ...
    } else if (ch == QLatin1String(gcs::ch::kHealth)) {
    ...
    } else if (ch == QLatin1String(gcs::ch::kMap)) {
        emit mapReceived(env.payload, p);
    } else if (ch == QLatin1String(gcs::ch::kLog)) {
...
void BridgeClient::setCmdVel(double vx, double vy, double wz)
{
    ...
    publish(QLatin1String(gcs::ch::kCmdVel),
            {{"vx", vx}, {"vy", vy}, {"wz", wz}});
}

void BridgeClient::requestGoal(double x, double y, double theta)
{
    sendRequest(QLatin1String(gcs::ch::kCmdGoto), {{"x", x}, {"y", y}, {"theta", theta}});
}

void BridgeClient::cancelNav()
{
    sendRequest(QLatin1String(gcs::ch::kCmdNavCancel));
}
...
{
    sendRequest(QLatin1String(gcs::ch::kCmdMissionStart), {{"from_index", 0}});
}

void BridgeClient::missionPause()
{
    sendRequest(QLatin1String(gcs::ch::kCmdMissionPause));
}

void BridgeClient::missionResume()
{
    sendRequest(QLatin1String(gcs::ch::kCmdMissionResume));
}

void BridgeClient::missionStop()
{
    sendRequest(QLatin1String(gcs::ch::kCmdMissionStop));
}
...
void BridgeClient::engageEstop()
{
    ...
    sendRequest(QLatin1String(gcs::ch::kCmdEstop));
    estop_ = true;
...
{
    sendRequest(QLatin1String(gcs::ch::kCmdEstopRelease), {{"confirm", true}});
}

void BridgeClient::setMode(DriveMode mode)
{
    mode_ = mode;
    sendRequest(QLatin1String(gcs::ch::kCmdMode),
...
void BridgeClient::setArmJointGoal(const QList<double> &q)
{
    ...
    for (double v : q)
        arr.append(v);
    sendRequest(QLatin1String(gcs::ch::kCmdArmJointGoal), {{"positions", arr}});
}

void BridgeClient::setArmPreset(const QString &name)
{
    sendRequest(QLatin1String(gcs::ch::kCmdArmPreset), {{"name", name}});
}

void BridgeClient::stopArm()
{
    sendRequest(QLatin1String(gcs::ch::kCmdArmStop));
}
