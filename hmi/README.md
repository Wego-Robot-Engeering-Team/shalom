...
| 옵션 | 기본 | 설명 |
|---|---|---|
| `HMI_REQUIRE_LOGIN` | `ON` | 기동 시 조작자 로그인 요구 |
| `HMI_BUILD_TESTS` | `ON` | 단위 테스트 빌드 |
...
개발 빌드의 기본은 **테스트베드 시뮬레이터**다. 명령을 실제로 처리한다 —
비상정지가 실제로 정지시키고, 수동 조작으로 로봇이 움직이며, 지도에서 목표를
찍으면 그리로 주행한다.

납품 빌드에는 시뮬레이터가 들어가지 않는다(`HMI_WITH_TESTBED=OFF`). 로봇
주소 없이 켜면 뜨지 않고 그렇게 말한다 — 현장에서 가짜 데이터로 도는 화면을
진짜로 오해하는 것보다 낫다.

`--live` 를 주면 설정된 주소의 브릿지에 raw TCP 로 접속한다.

```bash
./build/inspection_hmi --live
```
...
```bash
./build/inspection_hmi --dark --view arm
./build/inspection_hmi --no-login --shot /tmp/screen.png
```
...
```
INSPECTION-HMI-1.0.0-linux-x86_64/
├── inspection_hmi.sh               실행 스크립트 (LD_LIBRARY_PATH 지정)
├── bin/inspection_hmi
├── lib/                        libQt6*.so.6 · libhmi_core.so
...
|---|---|---|
| Windows | 레지스트리 `HKCU\Software\WEGO Robotics\Inspection HMI` | 실행 파일 옆 |
| Ubuntu | `~/.config/WEGO Robotics/Inspection HMI.conf` | 실행 파일 옆 |
