...
class EventLogPanel : public QWidget {
    Q_OBJECT
public:
    explicit EventLogPanel(hmi::diag::LogStore *store, QWidget *parent = nullptr);

    Card *card() const { return card_; }

private slots:
    void onAppended(const hmi::diag::LogEntry &e);
    void rebuild();
    void exportLog();

private:
    void addRow(const hmi::diag::LogEntry &e);
    void updateBadge();

    hmi::diag::LogStore *store_ = nullptr;
    Card *card_ = nullptr;
