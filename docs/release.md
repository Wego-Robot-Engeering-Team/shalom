...
**HMI 형상 데이터는 해결했다.** 고객 HMI는 제조사 STL·URDF 메쉬나 그 변환본을
포함하지 않는다. `Robot3DView`가 Wego가 작성한 박스·원통형 절차 생성 모델로
이동 베이스와 팔 자세를 그린다. `robot_mesh.bin`과 그 제조 도구는 제거했다.

`Kinematics.cpp`의 관절 원점·가동 범위는 실제 장착 후 교정해야 하며, 로봇 측
제어기가 최종 안전·가동 범위 판단을 맡는다.
