...
### 6.1a 라이브뷰 — 현재 범위 밖 (판단 근거 보존)

**관제 UI 의 라이브 영상은 구현하지 않는다.** 과업지시서가 요구하는 것은
「촬영 결과 즉시 미리보기」(요구사항 총괄표 ⑨)이며, 촬영은 정지 상태에서만
수행하므로(2.2.4) 연속 영상이 필요하지 않다.

문서에서 라이브 뷰가 등장하는 유일한 문장은 2.2.7 [4] 의 "카메라 라이브 뷰에서
점검 대상 부품을 AI가 자동 감지" 인데, 주어가 **AI 분석 PC** 다. 그 스트림은
로봇에서 AI PC 로 직접 가며 관제를 경유하지 않는다(§6.1b).

아래는 나중에 라이브뷰가 필요하다고 결정될 경우를 위한 검토 결과다.

**후보 비교** — 검수고 무선 실효 대역폭 10~30 Mbps 가정

MJPEG 는 프레임 간 중복을 제거하지 않아 해상도에 따라 대역폭이 급격히 나빠진다.
검수고 무선 실효 대역폭을 10~30 Mbps 로 가정할 때:

| 방식 | 640×480 @10fps ×2대 | 1280×720 @15fps ×2대 |
|---|---|---|
| MJPEG | 4~6 Mbps | 20~30 Mbps |
| H.264 | 1~2 Mbps | 3~4 Mbps |

MJPEG 를 우선 검토했던 이유:

1. **라이브뷰의 목적이 정밀 판독이 아니다.** 대상이 화면에 들어왔는지, 로봇이
   멈췄는지 확인하는 용도이고 정밀 판독은 촬영 원본으로 한다. 640×480 이면 족하다.
2. **의존성이 없다.** H.264 디코딩은 관제 PC 에 FFmpeg 계열 의존성을 들인다.
   에어갭 재빌드·임치 의존성 목록·1년 하자보수 관점에서 무거운 비용이다.
3. **포트 하나, HTTP.** RTSP 는 제어(TCP)와 데이터(RTP/UDP) 가 나뉘어 포트가
   늘고 3장 패킷 캡처 검증이 복잡해진다.
4. **무선 끊김에 강하다.** 프레임이 독립적이라 놓쳐도 다음 프레임에서 복구된다.
   H.264 는 키프레임을 놓치면 화면이 깨진 채 유지되는데, 열차 하부처럼 신호가
   출렁이는 구간에서 실제로 발생한다.

**H.264/RTSP 를 택해야 할 조건**

- 라이브뷰 해상도 요구가 720p 이상으로 확정되거나
- 현장 무선 실측 대역폭이 10 Mbps 미만이거나
- Jetson AGX Orin 의 NVENC 하드웨어 인코더 이점이 관제측 디코더 의존성 비용을
  넘는다고 판단될 때
...
| 항목 | 값 |
|---|---|
| 해상도 | 640×480 (필요 시 800×600) |
| 프레임률 | 10~15 fps |
| JPEG 품질 | 70~80 |
| 스트림당 대역폭 | 2~3 Mbps 이내 |

도입하더라도 **선택 기능으로 두고 기본은 꺼 둔다.** 아무도 보지 않는 영상에
무선 대역폭을 쓸 이유가 없고, 그 대역폭은 촬영 원본 업로드가 써야 한다.
