...
> 관제 UI(`inspection_hmi`, Windows/Ubuntu, Qt) 사이의 유일한 통신 규약이다.
...
| `robot` | str | ⬜ | 로봇 식별자. §1.2.1 |
| `p` | object | ⬜ | 페이로드 |

#### 1.2.1 로봇 식별자 (`robot`)

로봇이 자기 식별자를 실어 보낸다. 관제는 **그 연결에서 처음 들은 값을
고정하고**, 같은 연결에 다른 식별자가 오면 연결을 끊고 `E_ROBOT_MISMATCH`
를 올린다. 관제가 보내는 명령에도 아는 식별자를 실어, 로봇 쪽에서도 자기
앞으로 온 명령인지 확인할 수 있게 한다.

비워도 된다. 비어 있으면 "이 연결의 로봇" 을 뜻한다.

**지금 로봇이 한 대인데 왜 넣는가.** 나중에 넣으면 이미 납품된 브릿지와
호환이 깨지지만, 쓰지 않는 필드는 몇 바이트로 끝난다. 게다가 한 대일 때도
값을 한다 — 주소를 잘못 적어 옆 로봇에 붙어도 화면은 정상으로 보이는데,
그 상태로 비상정지를 누르면 엉뚱한 로봇이 선다. 식별자가 있으면 붙는
순간 걸린다.

로봇이 여러 대가 되면 관제는 로봇마다 연결을 하나씩 열고 이 필드로
구분한다. 규약은 그대로다.
...
| `sub` | 관제 → 브릿지 | `res` | 채널 구독. `p.channels: [str]` |
| `unsub` | 관제 → 브릿지 | `res` | 구독 해제 |
| `pub` | 양방향 | 없음 | 스트림 발행. 손실 허용 |
| `req` | 관제 → 브릿지 | `res` | 명령 요청. `id` 필수 |
| `res` | 브릿지 → 관제 | — | 요청 응답. `p.ok: bool`, 실패 시 `p.err: {code,msg}` |
| `evt` | 브릿지 → 관제 | 없음 | 비동기 이벤트(로그, 경고). 구독 무관 항상 수신 |
...
| `state/system` | 1 Hz | `cpu_pct, gpu_pct, mem_pct, cpu_temp_c, gpu_temp_c, net_rtt_ms, net_rssi` |
...
| `cmd/mission/stop` | `{}` | 점검을 끝낸다. 지점별 진행 상황을 모두 `todo` 로 되돌리므로, 다음 `start` 는 1번 지점부터다. 이어서 하려면 `pause`/`resume` 을 쓴다 |
...
### 4.1 안전 노드는 명령 경로 안에 있어야 한다 (로봇측 구현 지침)

지시서 2.2.5 는 "`/cmd_vel` 토픽 차단" 을 요구한다. ROS 2 에서 토픽을 실제로
차단할 수는 없으므로, 옆에서 0 을 더 자주 발행하는 방식은 **차단이 아니라
경쟁**이고 안전 논거가 되지 못한다.
...
**브릿지와 분리하는 이유**: 정지 판단이 브릿지 안에 있으면 브릿지의 크래시가
곧 감시자 없는 주행이 된다. 안전 노드는 브릿지가 발행하는 생존 신호의
**부재**를 근거로 삼으므로, 브릿지가 죽으면 침묵이 곧 정지가 된다.
같은 이유로 두 노드를 같은 launch 로 묶지 않는다.

**재개는 항상 명시적이어야 한다.** 통신이 복구되었다고 게이트를 자동으로
열면 안 된다. 그동안 Nav2 는 계속 돌고 있었으므로, 게이트가 열리는 순간
로봇이 앞으로 튀어나간다. 조작자가 화면에서 눈을 뗀 이유가 무엇이었든
그 상황에서 일어나서는 안 되는 일이다.

**기동 직후에는 게이트가 닫혀 있어야 한다.** 관제 소식을 한 번도 듣지 못한
상태에서 주행 가능한 로봇은 허용되지 않는다.
...
| 관제 하트비트 소실 판정 | 브릿지가 600 ms 미수신 → `heartbeat_ok:false` |
| 정지 발동 | safety 노드가 **3초** 미수신 시 현 위치 정지 + 경고음 (지시서 요구) |
| 브릿지 하트비트 소실 | 관제가 1.5초 미수신 → 연결 끊김 표시, 재연결 시도 |
...
경로가 둘이고 목적이 다르다. 하나로 합치려 하면 둘 다 나빠진다.
...
관제 UI 는 실시간 영상을 받지 않는다. 과업지시서가 요구하는 것은 「촬영 결과
즉시 미리보기」(요구사항 총괄표 ⑨)이고, 촬영은 정지 상태에서만 수행하므로
(2.2.4) 연속 영상이 필요한 장면이 없다. 미리보기 썸네일은 제어 소켓의
`capture/preview` 로 충분하다.

라이브뷰를 넣으면 관제 PC 에 영상 디코더 의존성이 생기고, 그것은 에어갭
재빌드와 임치 의존성 목록에 그대로 얹힌다. 요구되지 않는 기능 때문에 치를
비용이 아니다.

나중에 요구가 생기면 제어 소켓과 별개의 HTTP 경로로 붙인다 — 제어 채널에
영상을 실으면 명령 응답이 영상 뒤에 밀린다.

### 6.1b AI 분석 PC 스트림은 관제를 경유하지 않는다

비전 기반 대상물 자동 인식(지시서 2.2.7 [4])을 위한 스트림은 로봇에서 AI 분석
PC 로 직접 보낸다. 관제용 라이브뷰와 화질·프레임률 요구가 다를 수 있고,
관제를 중계로 쓰면 무선 구간을 두 번 타기 때문이다. 촬영 원본을 NAS 로 직접
보내는 것과 같은 이유다.
