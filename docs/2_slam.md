...
```bash
ros2 run nav2_map_server map_saver_cli -f ~/shalom_ws/src/shalom/application/maps/$(date +%F)
```
...
높이 밴드는 `application/config/ground_filter_b2.yaml`에서 조정한다.
...
지면 판정 자체는 `application/config/gseg3d_b2.yaml`이다. **`lidar_to_ground`는
