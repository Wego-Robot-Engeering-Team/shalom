...
```cmake
add_subdirectory(path/to/api)
target_link_libraries(my_app PRIVATE shalom::api)
```

`shalom::api` 가 `ws2_32` 를 함께 걸어 주므로 따로 적지 않아도 된다.

CMake 를 쓰지 않는다면 포함 디렉터리에 `api\include` 를 넣고 `ws2_32.lib` 를
링크한다.
